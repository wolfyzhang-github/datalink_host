...
| `remote_host` / `remote_port` | `169.254.56.252:3677` | client 模式设备地址。 |
...
| 参数 | 默认值 | 作用 |
|---|---:|---|
| `enabled` | `False` | 是否启用 GNSS 时间。 |
| `port` | 空字符串 | 串口设备名；为空时服务不会启动读取线程。 |
| `baudrate` | `115200` | 串口波特率。 |
| `mode` | `debug` | `debug` 或 `deploy`，决定时间格式和是否主动轮询。 |
| `poll_interval_seconds` | `0.1` | debug 模式向设备发送命令的间隔。 |
| `timestamp_interval_seconds` | `0.1` | 旧版 cadence 配置，当前包结束授时逻辑不再用于帧打点。 |
...
```text
gnss_packet_end_us = GnssTimeService.wait_for_next_timestamp_us(packet_timestamp_timeout_seconds)
```
...
若在 `packet_timestamp_timeout_seconds` 内没有等到 GNSS 包结束时间戳，但 Runtime 已有上一帧时间状态：
...
若 GNSS 不可用且没有上一帧可兜底，则 `timestamp_us = None`。该帧仍会进入处理管线和 GUI 最新波形，但存储和 DataLink 会跳过输出。
...
- `root` 为输出根目录。
- `file_duration_seconds` 决定单文件样本数：`round(sample_rate * file_duration_seconds)`。
...
每次写出一个 MiniSEED chunk 后，程序也会在同一输出目录写出一份同起始时间的日志 sidecar 文件：

```text
{network}.{station}.{YYYYMMDDHHMMSSmmm}.R.{location}.LOG.log
```

日志文件内容来自程序内存日志缓冲，并带有该 MiniSEED chunk 的起止 UTC 时间头。
...
判断当前磁盘可用空间。如果 `free_bytes < required_bytes`，会在 `root` 下递归查找最旧的 `*.mseed` 文件并删除，同时删除同名时间戳的 `.LOG.log` sidecar；删除后重新检查空间，直到新 MiniSEED 和 LOG 可以写入。如果已经没有可删除的 MiniSEED 文件仍空间不足，则本次写入抛出磁盘空间不足错误并进入存储 sink 的错误统计。
...
- `start_time = frame.timestamp_us / 1_000_000`。
...
8. DataLink 节点每个 `ProcessedFrame` 都直接用该帧的 `timestamp_us` 作为 Data1/Data2 payload 起始时间。如果降采样器存在 `_carry`，某个 `ProcessedFrame.data1/data2` 的第一批输出样本可能包含上一帧遗留样本；此时 DataLink 的起始时间可能晚于这些输出窗口的真实起点。

9. 与上一条相关，MiniSEED 本地存储虽然后续按连续时间推进，但如果某一路一开始若干帧都因为 carry 不足而没有输出，等第一次真正有降采样输出时，它会用“当前源帧”的 `timestamp_us` 锚定；这些输出可能实际包含更早帧的 carry，初始锚点也可能偏晚。
