...
| `port` | 空字符串 | 串口设备名；为空时启动/配置更新会在只有一个可见串口时自动选择。 |
...
GNSS 服务启动条件是 `gnss.enabled == True` 且 `gnss.port` 非空。若启用但端口为空，Runtime 会先尝试在系统只有一个可见串口时自动选择该端口；连接串口后会先丢弃已有串口输入缓冲，避免旧时间戳污染后续帧。
...
常规 `timeout_seconds` 取 `packet_timestamp_timeout_seconds` 与串口读取超时的较大值。冷启动时按现场链路时序额外保护：GNSS 约 40 秒可用、TCP 数据约 80 秒可用，程序会把第一次授时等待窗口补齐到启动后 90 秒（80 秒 + 10 秒余量）。数据连接打开或 GNSS 配置更新后的第一次授时等待也会额外按 `timestamp_interval_seconds` 放宽一次；如果已超过冷启动保护窗口，则回到常规等待。这个初始等待只消费一次；如果 GNSS 仍然没有到达，后续包回到常规超时和连续时间兜底。
...
如果 GNSS 开关启用但串口端口为空且自动选择不到端口，Runtime 不会等待 GNSS 包，而是使用 TCP 包接收时间作为帧结束时间，并标记 `gnss_fallback_active=True`。
...
```text
{root}/{YYYYMMDD}/data1-01/
{root}/{YYYYMMDD}/data1-02/
...
{root}/{YYYYMMDD}/data2-01/
...
{root}/{YYYYMMDD}/log/
```
...
每次追加写出一个 MiniSEED chunk 后，程序也会在当天 `{root}/{YYYYMMDD}/log/` 目录写出一份同文件窗口起始时间的日志 sidecar 文件：
