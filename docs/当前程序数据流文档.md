...
| `port` | 空字符串 | 串口设备名；为空时启动/配置更新会尝试自动选择第一个可见串口。 |
...
GNSS 服务启动条件是 `gnss.enabled == True` 且 `gnss.port` 非空。若启用但端口为空，Runtime 会先尝试从系统串口列表自动选择第一个端口；连接串口后会先丢弃已有串口输入缓冲，避免旧时间戳污染后续帧。
