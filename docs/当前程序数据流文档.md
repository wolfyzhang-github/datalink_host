...
常规 `timeout_seconds` 取 `packet_timestamp_timeout_seconds` 与串口读取超时的较大值。启动后的第一帧如果还没有任何 GNSS 时间戳，会额外按 `timestamp_interval_seconds` 放宽一次等待窗口，避免程序刚启动、串口线程尚未读到首条授时时误报。
...
每次写出一个 MiniSEED chunk 后，程序也会在与数据目录并列的 `{root}/log/` 目录写出一份同起始时间的日志 sidecar 文件：
